...
balanceA = MatchBalance(treat~ ave_CAHM + CHAMchallenge + SIS_hand + ufugm + 
           ...
           ,
               ...
               data= data12, match.out= mgen1, nboots=50000)
...
```{r}
mgen1=Match(Y= data12$logwmftMA_recovery,BiasAdjust = TRUE, estimand= "ATT", Tr=data12$treat, X=X1, Weight.matrix = gen1)
...
#Additional way of checking balance
...
(bal.tab(mgen1, treat~ ave_CAHM + CHAMchallenge + SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               ...
               race.asian+ race.africanam+ race.caucasian + race.other+ 
               edu.noHSdegree +edu.finishHS +edu.someCollege + edu.finishCollege+ 
               stroke.loc.RightH+ stroke.loc.LeftH +stroke.loc.cerebellar +stroke.loc.brainstem +          M =                propensity.score,

         disp.v.ratio = TRUE, data = data12,abs= TRUE, m.threshold =0.3))
```
