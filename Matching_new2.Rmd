...
# propensity score excluded- last covariate weight = 0 not 0.90919961 or 1
# removing 57th weight and propensity score improves p value for genetic matching
...
rr1 =Match(Tr= data12$treat, X= model$fitted.values, estimand = "ATT", 
           M=1, BiasAdjust = TRUE, ties = TRUE, replace= TRUE)

# NOTE ON TIES = TRUE
# ties- A logical flag for whether ties should be handled deterministically. By default
# ties==TRUE. If, for example, one treated observation matches more than one
# control observation, the matched dataset will include the multiple matched control observations and the matched data will be weighted to reflect the multiple
# matches. The sum of the weighted observations will still equal the original
# number of observations. If ties==FALSE, ties will be randomly broken. If the
# dataset is large and there are many ties, setting ties=FALSE often results in a
# large speedup. Whether two potential matches are close enough to be considered tied, is controlled by the distance.tolerance option.
```
...
MatchBalance(treat~ ave_CAHM + CHAMchallenge + SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               ...
               stroke.loc.RightH+ stroke.loc.LeftH +stroke.loc.cerebellar +stroke.loc.brainstem+
...
```{r}
set.cobalt.options(continuous= "std",binary= "std", disp.means= TRUE)
...
(bal.tab(rr1, treat~ ave_CAHM + CHAMchallenge + SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
         ...
         disp.v.ratio = TRUE, data = data12,abs= TRUE, m.threshold =0.2))
...
gen1 = GenMatch(Tr= data12$treat, 
                ...
                X=X1, pop.size=1000, replace=TRUE, max.weight=1)
