...
dataVisit3= dataVisit3[complete.cases(dataVisit3[,4:5]),]
```
...
### Also Removing visit1 and visit 2 data with no available visit 3 features
```{r}
temp= data.frame(dataVisit3$pid); 
names(temp)[1]<-paste("pid");   #renaming column name to "pid"
dataVisit1= merge(temp,dataVisit1)
...
### Removing visit3 data with no available visit 1/2 features
```{r}
temp= data.frame(dataVisit1$pid); 
names(temp)[1]<-paste("pid");   #renaming column name to "pid"
...
```{r}
data12$treat=ifelse(dataVisit1$dose_hours<=Lthreshold, 1, 0)
```
...
X= cbind(data12$SIS_hand, data12$ufugm,data12$CSgender,data12$NIHtot, 
         ...
         data12$ischemic,data12$A1EstVol, data12$old_stroke, data12$multlesions)
...
MatchBalance(treat~ SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               ...
               onset_to_rand + finished_college + age_at_rand +ischemic +
               A1EstVol+ old_stroke+ multlesions+propscore,
             match.out= rr1, nboots=100,data= datap)
...
#### Assessing balance
```{r}
print(MatchBalance(treat~ SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               log_mean_time_LA_PA +CSgender+ concordance + severity + 
