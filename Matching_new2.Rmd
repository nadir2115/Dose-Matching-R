...
### No longer imputing values. Just removing data point with missing values
...
### Normalization not necessary because genetic matching involves a modified mahalanobis distance- normalize by subtracting mean and dividing by std (doing min-max instead)
```{r}
...
rawd=rename(rawd, pid= Study_id ) #renaming column name
...
#### WE WILL NOT USE THE LAST ICARE UPDATE DATA-SET AS IT WILL SLASH our data-points by 60%
```{r}
# icareupdate= read.csv("icare results update.csv")
# names(icareupdate)[1]<-paste("pid");   #renaming column name to "pid"
# 
# temp= data.frame(icareupdate$pid)
# names(temp)[1]<-paste("pid");   #renaming column name to "pid"
# 
# datarm= merge(temp,datarm, by = "pid")
...
```{r}
data12$propscore= model$fitted.values

library(gridExtra)

plot1<-ggplot(data12, aes(propscore)) +
...
plot2<-ggplot(data12, aes(propscore)) +
...
MatchBalance(treat~ SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               ...
               LeftH.Stroke + RightH.Stroke + propscore,
...
MatchBalance(treat~ SIS_hand + ufugm + NIHtot + log_mean_time_MA_PA +
               ...
               old_stroke+ multlesions + propscore,
...
```{r}
for (i in c(2,4,5,12,14,16,18,19)){
  string= columns[i]
...
## Ttests
```{r}
t.test(data12$logwmftMA_change[mgen1$index.treated],data12$logwmftMA_change[mgen1$index.control])
t.test(data12$sishand_change[mgen1$index.treated],data12$sishand_change[mgen1$index.control])

t.test(data12$logwmftMA_retentionv23[mgen1$index.treated],data12$logwmftMA_retentionv23[mgen1$index.control])
t.test(data12$sishand_retentionv23[mgen1$index.treated],data12$sishand_retentionv23[mgen1$index.control])

```

```{r}
hist(data12$logwmftMA_change[mgen1$index.treated])
hist(data12$logwmftMA_change[mgen1$index.control])
shapiro.test(data12$logwmftMA_change[mgen1$index.treated])

# 
# install.packages("PerformanceAnlaytics")
# library("PerformanceAnalytics")
# chart.Correlation(data12)
